...
# ==============================
df = df.withColumn("event_time", to_timestamp("time_generated"))

df = df.withWatermark("event_time", "5 seconds")
...
session_df = df.groupBy(
    ...
    collect_set(when(col("event_type") == "add_to_cart", col("product_id"))).alias("cart_products"),
...
kafka_df = session_df.select(
    ...
    to_json(struct(
        col("user_id"),
        col("cart_products").alias("product_ids"),
        col("session_window.end").alias("ts")
    )).alias("value")
).filter(
    (size(col("cart_products")) > 0) & (col("has_purchased") == 0)
)
